  ...
  <tr class="row">
    ...
    <td class="row__item result">
      <button v-if="!isError"
              ...
              @click="open"
              :disabled="!isOpen">Открыть</button>
      <button class="result__btn result__btn_remove">Удалить</button>
    </td>
  </tr>
</template>
...
export default {
  ...
  async mounted() {
    ...
    this.pollData(this.checkProgress, async (res) => {
      ...
      this.$emit('recognition-finished', { name: formData.get('key'), data: recognitionData})
